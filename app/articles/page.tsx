...
export const metadata: Metadata = {
  ...
  description: "บทความตัวอย่างเกี่ยวกับกายภาพบำบัด อาการปวด Running Rehabilitation และ Return to Sport จาก TREHAB Physiotherapy Clinic เชียงราย",
...
export default function ArticlesPage() {
  return <main className="trehab-articles-page" id="top">
    ...
    <section className="trehab-articles-list" aria-labelledby="article-list-title"><div className="shell"><div className="trehab-articles-heading"><div><p className="section-kicker">LATEST ARTICLES</p><h2 id="article-list-title">บทความตัวอย่าง</h2></div><p>เนื้อหาทั้ง 3 บทความจัดทำเป็นตัวอย่างสำหรับเว็บไซต์ TREHAB และใช้ภาษาที่เน้นการประเมินและการฟื้นฟูอย่างเหมาะสม</p></div><div className="trehab-article-grid">{articles.map((article,index)=><article className="trehab-article-card" key={article.slug}><Link className="trehab-article-image" href={`/articles/${article.slug}`} aria-label={`อ่าน ${article.title}`}><Image src={article.image} fill sizes="(max-width: 760px) 100vw, 33vw" alt={article.imageAlt} loading="lazy"/></Link><div className="trehab-article-card-content"><div className="trehab-article-meta"><span>{String(index+1).padStart(2,"0")}</span><b>บทความตัวอย่าง</b></div><p className="trehab-article-category">{article.category}</p><h3><Link href={`/articles/${article.slug}`}>{article.title}</Link></h3><p>{article.excerpt}</p><div className="trehab-article-card-footer"><span>{article.readingTime}</span><Link href={`/articles/${article.slug}`}>อ่านบทความ <span aria-hidden="true">→</span></Link></div></div></article>)}</div></div></section>
