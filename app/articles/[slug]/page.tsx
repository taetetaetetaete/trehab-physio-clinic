...
export default async function ArticlePage({ params }: PageProps) {
  ...
  return <main className="trehab-article-detail" id="top">
    ...
    <article>
      <header className="trehab-article-detail-header"><div className="shell"><div className="trehab-article-breadcrumb"><Link href="/articles">บทความ</Link><span aria-hidden="true">/</span><span>{article.category}</span></div><p className="section-kicker">{article.category}</p><h1>{article.title}</h1><p className="trehab-article-lead">{article.excerpt}</p><div className="trehab-article-byline"><span>บทความตัวอย่าง</span><span>{article.readingTime}</span><span>ปรับปรุงล่าสุด 31 สิงหาคม 2569</span></div></div></header>
